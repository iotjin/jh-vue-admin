import Vue from 'vue'
import TableColumnSettingDialog from './TableColumnSettingDialog.vue'

const DialogConstructor = Vue.extend(TableColumnSettingDialog)
const TABLE_SETTING_KEY = '__tableColumnSettingState__'
 ...
 * 将内部列结构转成弹框可消费的数据结构
 */
function toDialogColumns(columns) {
...
function ensureToolbar(el, state) {
  ...
  button.innerHTML = '<span>列设置</span>'
  button.addEventListener('click', state.onOpenDialog)
 ...
 */
function ensureDialog(state, vm) {
  if (state.dialogVm) {
    return
  }
  const mountNode = document.createElement('div')
  document.body.appendChild(mountNode)
  const dialogVm = new DialogConstructor({
    propsData: {
      visible: false,
      columns: [],
      defaultColumns: []
  ...
  dialogVm.$mount(mountNode)

  dialogVm.$on('update:visible', (val) => {
    // 子组件 v-model 同步
    dialogVm.visible = val
  })
  dialogVm.$on('confirm', (columns) => {
    // 只有确认才真正写回表格列状态
    applyColumnSetting(state, columns, vm)
  })

  state.dialogVm = dialogVm
  state.dialogMountNode = mountNode
}
...
function syncColumnsFromTable(state, vm) {
  ...
  if (!state.allColumns.length) {
    ...
    state.defaultColumns = toDialogColumns(normalizedStoreColumns)
    return state.allColumns
  }
...
function openDialog(state, vm) {
  // 打开前先从表格回填最新状态，确保弹框展示与表格一致
  const currentColumns = syncColumnsFromTable(state, vm)
  state.dialogVm.columns = toDialogColumns(currentColumns)
  state.dialogVm.defaultColumns = toDialogColumns(state.defaultColumns)
  state.dialogVm.visible = true
}

/**
 * 以弹框返回结果为唯一真值，更新表格列显隐/顺序/固定
 */
function applyColumnSetting(state, dialogColumns, vm) {
  if (!vm || !vm.store || !vm.store.states || !Array.isArray(dialogColumns)) {
    return
  }
  const columnMap = new Map(state.allColumns.map((item) => [item.id, item]))
  const nextColumns = []

  dialogColumns.forEach((item) => {
...
function destroyState(el) {
  ...
  if (state.buttonEl) {
    state.buttonEl.removeEventListener('click', state.onOpenDialog)
  }
  ...
  if (state.dialogVm) {
    state.dialogVm.$destroy()
  }
  if (state.dialogMountNode && state.dialogMountNode.parentNode) {
    state.dialogMountNode.parentNode.removeChild(state.dialogMountNode)
  }
  delete el[TABLE_SETTING_KEY] // 清理挂载状态，防止内存泄漏
}

const TableColumnSettingDirective = {
  inserted(el, binding, vnode) {
    ...
    const state = {
      ...
      buttonEl: null,
      dialogVm: null,
      dialogMountNode: null,
      onOpenDialog: null
    }
    state.onOpenDialog = () => openDialog(state, vm)
...
/**
 * 全局注册入口：
 * 在 src/directives/index.js 中调用后，可在任意页面直接使用 v-table-column-setting。
 */
export const tableColumnSettingDirective = (VueInstance) => {
  VueInstance.directive('table-column-setting', TableColumnSettingDirective)
}

export default TableColumnSettingDirective
